...
def enviar_histogramas_prometheus(df, fecha="", ip_pushgateway="localhost:9091"):
    ...
    registro = CollectorRegistry()
    metrica_preflen = Gauge('rpki_roa_prefixlen_count', 'Cantidad de ROAs por longitud de prefijo',
                            ['version', 'longitud', 'trust_anchor', 'fecha'], registry=registro)
    metrica_maxlen  = Gauge('rpki_roa_maxlen_count',    'Cantidad de ROAs por MaxLength del ROA',
                            ['version', 'longitud', 'trust_anchor', 'fecha'], registry=registro)
        ...
        for columna, metrica in [('Prefijo_Base', metrica_preflen), ('Max_Length', metrica_maxlen)]:
            ...
            for longitud in pivot.index:
                for ta in pivot.columns:
                    metrica.labels(version=version, longitud=f"{int(longitud):03d}",
                                   trust_anchor=ta, fecha=fecha).set(int(pivot.loc[longitud, ta]))
